...
const Preview = ({ selectedParty, selectedQualities, challanNumber, kg, meter, roll }) => {
  return (
    <div className="border-login border-[1px] rounded-xl overflow-hidden mt-8">
      ...
      <div className="ml-4 mt-4">
        ...
        <p className="text-gray-700 mb-2">Qualities: {selectedQualities.join(', ')}</p>
  ...
  return (
    ...
    <div className="relative mt-4 ml-4 mb-4">
      <div className="inline-block relative">
        ...
        >
          Select Qualities
          <svg className="absolute inset-y-0 right-0 w-4 h-4 mt-3 mr-4 pointer-events-none fill-current text-gray-700" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20"><path fillRule="evenodd" d="M10 12a2 2 0 100-4 2 2 0 000 4z" clipRule="evenodd"/></svg>
        </button>
        {isOpen && (
                ...
                <label key={index} className="inline-flex items-center mt-1">
                  <input
                    ...
                    onChange={(e) => {
                      ...
                      if (e.target.checked) {
                        setSelectedQualities((prevSelected) => [...prevSelected, value]);
                      } else {
                        setSelectedQualities((prevSelected) => prevSelected.filter((item) => item !== value));
                      }
                    }}
                    className="mr-2"
                  />
    ...
    {selectedQualities.length > 0 && (
      <div className="mt-4 ml-4">
        <p className="text-gray-700 mb-2 font-login">Selected Quality:</p>
        <ul className="flex flex-wrap">
          {selectedQualities.map((quality, index) => (
            <li key={index} className="bg-gray-200 text-gray-700 px-3 py-1 rounded-full mr-2 mb-2">
              {quality}
            </li>
          ))}
        </ul>
